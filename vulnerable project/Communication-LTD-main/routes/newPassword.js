...
router.post('/', async function (req, res) {
    ...
    console.log('password= ', bf.encrypt(currentPassword));
    const passwordMatch = bf.encrypt(currentPassword) === dbPassword;
    ...
    const previousPasswords = passwordHistory.slice(0, history - 1).map((password) => encodeURIComponent(bf.encrypt(newPassword)) === password);
    if (previousPasswords.includes(true)) {
    ...
    const previousPasswordHashes = passwordHistory.slice(0, history - 1).map((password) => {
        const encrypted = bf.encrypt(password);
        return bf.base64Encode(encrypted);
    });
    const currentPasswordEncrypted = bf.encrypt(newPassword);
    const currentPasswordHashed = bf.base64Encode(currentPasswordEncrypted);
